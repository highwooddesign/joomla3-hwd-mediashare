...
/**
 * @package     Joomla.site
 * @subpackage  Plugin.hwdmediashare.remote_vevocom
 *
...
class plgHwdmediashareRemote_vevocom extends hwdMediaShareRemote
{    
	/**
	 ...
	 *
	 * @access  public
	 * @return  object  The plgHwdmediashareRemote_vevocom object.
	 */
	...
	public function display($item)
	{
		...
		$plugin = JPluginHelper::getPlugin('hwdmediashare', 'remote_vevocom');
                ...
                $lang->load('plg_hwdmediashare_remote_vevocom', JPATH_ADMINISTRATOR, $lang->getTag());

                if (!$plugin)
                {
                        $this->setError(JText::_('PLG_HWDMEDIASHARE_REMOTE_VEVOCOM_ERROR_NOT_PUBLISHED'));
                        return false;
                }
                ...
                if ($embedLookup)
                {
                        ...
                        <div class="media-respond" style="max-width:<?php echo $config->get('mediaitem_size', '500'); ?>px;">
                          ...
                          <div class="media-content">
                            <iframe width="<?php echo $this->width; ?>" height="<?php echo $this->height; ?>" src="<?php echo $embedLookup; ?>" scrolling="no" frameborder="0" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true" allowtransparency="true"></iframe>
                          </div>
                        </div>
                        <?php
                ...
                {
                        $this->setError(JText::_('PLG_HWDMEDIASHARE_REMOTE_VEVOCOM_ERROR_PLAYBACK_PROBLEM_SEE_ORIGINAL'));
                        return false;
                }
	}
	...
	public function getDirectDisplayLocation($item)
	{
		...
		$plugin = JPluginHelper::getPlugin('hwdmediashare', 'remote_vevocom');
                ...
                {             
                        ...
                        // Get the embed parameters.
                        parse_str(parse_url($embedLookup, PHP_URL_QUERY), $parameters);
                        if (isset($parameters['video']))
                        {
                                return JURI::getInstance()->getScheme() .'://' . parse_url($embedLookup, PHP_URL_HOST) . parse_url($embedLookup, PHP_URL_PATH) . '?video=' . $parameters['video'] . '&autoplay=' . $this->autoplay;
                        }                  
                }
        }  
