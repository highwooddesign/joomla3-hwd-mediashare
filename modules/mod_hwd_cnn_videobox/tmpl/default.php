...
?>
<div class="row mod_hwd_cnn_videobox_default">
<?php foreach ($helper->items as $id => $item) : ?>
  ...
  <div class="cell col-xs-6 col-sm-4 col-md-<?php echo intval(12/$helper->params->get('columns', 3)); ?>">
    ...
    <?php endif; ?>
    <div class="clear"></div>
    <?php if ($helper->params->get('show_category') != 'hide'):?><span class="small badge pull-right"><?php echo $helper->getCategory($item->category); ?></span><?php endif; ?>
    <?php if ($helper->params->get('show_date') != 'hide'):?><span class="small"><?php echo JHtml::_('date.relative', $item->publishedDate); ?></span><?php endif; ?>
 </div>
 <?php endforeach; ?>
</div>
