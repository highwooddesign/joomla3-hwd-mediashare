<?php
/**
 * @version    $Id: default.php 1378 2013-04-23 12:50:17Z dhorsfall $
...
?>
      ...
      <div class="system-unpublished">
        ...
        <?php if ($params->get('list_meta_title') != 'hide') :?>
          <h<?php echo $params->get('list_item_heading'); ?> class="contentheading<?php echo ($params->get('list_tooltip_location') > '1' ? ' hasTip' : ''); ?>" title="<?php echo $helper->get('utilities')->escape($item->title); ?>::<?php echo ($params->get('list_tooltip_contents') != '0' ? $helper->get('utilities')->escape(JHtmlString::truncate($item->description, $params->get('list_desc_truncate'), true, false)) : ''); ?>">
            <?php if ($params->get('list_link_titles') == 1) :?><a href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getUserRoute($item->slug)); ?>"><?php endif; ?>
        ...
        <div class="media-item">
          ...
          <ul class="media-nav">
            <li><a href="#" class="pagenav-manage"><?php echo JText::_('COM_HWDMS_MANAGE'); ?> </a>
              <ul class="media-subnav">
                <?php if ($canEdit) : ?>
                <li><?php echo JHtml::_('hwdicon.edit', 'user', $item, $params); ?></li>
                <?php endif; ?>
                <?php if ($canEditState) : ?>
                <?php if ($item->published != '1') : ?>
                <li><?php echo JHtml::_('hwdicon.publish', 'user', $item, $params); ?></li>
                <?php else : ?>
                <li><?php echo JHtml::_('hwdicon.unpublish', 'user', $item, $params); ?></li>
                <?php endif; ?>
                <?php endif; ?>
                <?php if ($canDelete) : ?>
                <li><?php echo JHtml::_('hwdicon.delete', 'user', $item, $params); ?></li>
                <?php endif; ?>
              </ul>
            </li>
          </ul>
          ...
          <?php endif; ?>
          <?php if ($params->get('list_link_thumbnails') == 1) : ?><a href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getUserRoute($item->slug)); ?>"><?php endif; ?>
        ...
        <div class="clear"></div>
        <?php if ($params->get('list_meta_description') != 'hide' || $params->get('list_meta_created') != 'hide' || $params->get('list_meta_likes') != 'hide' || $params->get('list_meta_hits') != 'hide') : ?>
        <!-- Item Meta -->
        <dl class="article-info">
          ...
          <?php if ($params->get('list_meta_likes') != 'hide') :?>
            <dd class="media-info-like"> <a href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=user.like&id=' . $item->id . '&return=' . $helper->get('return') . '&tmpl=component'); ?>"><?php echo JText::_('COM_HWDMS_LIKE'); ?></a> (<?php echo $helper->get('utilities')->escape($item->likes); ?>) <a href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=user.dislike&id=' . $item->id . '&return=' . $helper->get('return') . '&tmpl=component'); ?>"><?php echo JText::_('COM_HWDMS_DISLIKE'); ?></a> (<?php echo $helper->get('utilities')->escape($item->dislikes); ?>) </dd>
          <?php endif; ?>
  ...
  <?php $counter++; ?>
  <?php endforeach; ?>
  <?php if ($params->get('show_more_link') != 'hide') :?><p><a href="<?php echo ((intval($params->get('show_more_link')) > 0) ? JRoute::_($menu->getItem($params->get('show_more_link'))->link.'&Itemid='.$params->get('show_more_link')) : JRoute::_(hwdMediaShareHelperRoute::getUsersRoute())); ?>"><?php echo JText::_($params->get('more_link_text', 'COM_HWDMS_VIEW_ALL')); ?></a></p><?php endif; ?>  
</div>
</div>