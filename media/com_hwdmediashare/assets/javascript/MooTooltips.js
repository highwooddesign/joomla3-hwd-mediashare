...
var MooTooltips = new Class({
	...
	options: {		
		...
		toolTipPosition:1, 	// 1 left; 2: right
		showDelay: 0,		// 500
	...
	attach: function(){
		this.elements.each(function(elem, key){
			...
			*/
			...
			if( t.position == 1 ) {
				tooltip.header.set({'class':'dockTopHeaderLeft'});
			}
			if( t.position == 2 ) {
				tooltip.header.set({'class':'dockTopHeaderRight'});
				tooltip.inside.set({
					'styles':{ 
						'float':'right',
					}
			...
			if( t.sticky || this.options.sticky){
				tooltip.close.addEvent('click', this.hide.pass(tooltip.container).bind(this)  );
                        ...
                        elem.addEvent('mouseleave', function(){
                            tooltip.container.addEvents({
                                mouseover: over,
                                mouseleave: out.pass(tooltip.container)
                            });
                            //elem.addEvent('mouseleave', out.pass(tooltip.container));
                        });
                        //elem.addEvent(endEvent, out.pass(tooltip.container,elem));
	...
	enter: function(event, element){
		...
		this.fromTop = elSize.top - this.options.fromTop;

		if( elProperties.position == 1 )
			this.fromLeft = elSize.left - tipSize.width + elSize.width;
		else
			this.fromLeft = elSize.left + this.options.fromLeft;
		
                var top_dist = this.fromTop + (elProperties.position||this.options.toolTipPosition) * this.options.fadeDistance;
		var left_dist = this.fromLeft;
	...
	createContainer: function( sticky ){
		...
		var inside = new Element('div').set({
			'class':this.options.ToolTipClass + '-inside'
		});
		...
		container.adopt( inside );
		inside.adopt( header, message, footer );
		
		return {'container':container,'inside':inside,'header':header,'message':message,'footer':footer,'close':closeBtn||null};		
	}	
});