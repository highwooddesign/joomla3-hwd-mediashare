<?php
/**
 * @version    SVN $Id: user.php 1647 2013-08-16 09:13:57Z dhorsfall $
...
class hwdMediaShareModelUser extends JModelAdmin
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_hwdmediashare.user.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	/**
	...
	{
		return 'administrator/components/com_hwdmediashare/models/forms/user.js';
	}
	/**
	 * Method to get the script that have to be included on the form
	 *
	...
	public function save($data)
	{
                ...
                // ** Important ** Need to check that an existing channel already has a key, incase channel was setup without a key.
                // Only apply a key when one doesn't exist.
                if ($data['id'] > 0)
                {
                        ...
                        empty($row->key) ? $data['key'] = hwdMediaShareFactory::generateKey() : null;
                }
                ...
                // Set created_user_id only only if saving from the administrator, or if saving new item.
                ...
                empty($data['alias']) ? $data['alias'] = '' : $data['alias'] = JFilterOutput::stringURLSafe($data['alias']);
		...
		if ($form) 
                {
                        ...
                        !$app->isAdmin() ? $data['id'] = $this->getState('userform.id') : $data['id'] = $this->getState('user.id');
	...
	public function batch($pks, $value = array())
	{
		...
		if (!isset($value['access']) || !isset($value['language']))
                {
