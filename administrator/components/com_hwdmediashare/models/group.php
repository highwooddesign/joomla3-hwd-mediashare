<?php
/**
 * @version    SVN $Id: group.php 1645 2013-08-16 09:13:27Z dhorsfall $
...
class hwdMediaShareModelGroup extends JModelAdmin
{
	...
	public function getItem($pk = null)
	{
                if ($item = parent::getItem($pk))
                {
                        ...
                        $item->membercount = $this->getMemberCount($item);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_hwdmediashare.group.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	/**
	...
	{
		return 'administrator/components/com_hwdmediashare/models/forms/group.js';
	}
	/**
	 * Method to get the script that have to be included on the form
	 *
	...
	public function getMemberCount($item)
	{
                ...
                $query = "
                  SELECT COUNT(*)
                    ...
                    WHERE ".$db->quoteName('group_id')." = ".$db->quote($item->id).";
                  ";
                $db->setQuery($query);
	...
	public function save($data)
	{
                ...
                if (!$app->isAdmin() && $config->get('approve_new_groups') == 1) 
                { 
                        $data['status'] = 2;
                }
                else
		...
		if ($form) 
                {
                        ...
                        !$app->isAdmin() ? $data['id'] = $this->getState('groupform.id') : $data['id'] = $this->getState('group.id');

                        $params = new StdClass;
                        $params->elementType = $this->elementType;
                        ...
                        if ($isNew)
                        {
                                ...
                                $events->triggerEvent('onAfterGroupAdd', $row);
                        }
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
                        ...
                        $queries[] = "
                            DELETE
                                FROM ".$db->quoteName('#__hwdms_group_invite')."
                        ...
                        // Delete records from group members
                        $queries[] = "
                            DELETE
                                FROM ".$db->quoteName('#__hwdms_group_members')."
                                WHERE ".$db->quoteName('group_id')." = ".$db->quote($pk)."
