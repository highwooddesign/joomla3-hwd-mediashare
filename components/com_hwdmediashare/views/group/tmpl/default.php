  ...
  <div id="hwd-container"> <a name="top" id="top"></a>
    ...
    <div class="media-header">
      ...
      <div class="btn-group pull-right">
        <?php if ($canAddMedia): ?>
          <a title="<?php echo JText::_('COM_HWDMS_ADD_MEDIA'); ?>" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&view=upload&tmpl=component&group_id='.(int)$this->group->id); ?>" class="btn"><i class="icon-plus"></i> <?php echo JText::_('COM_HWDMS_ADD_MEDIA'); ?></a>
        <?php endif; ?>
        ...
        <?php endif; ?>          
        <?php if ($this->group->featured) : ?>
          <div class="btn btn-info"><i class="icon-star"></i> <?php echo JText::_('COM_HWDMS_FEATURED'); ?></div>
        <?php endif; ?> 
        <?php if ($this->group->status != 1) : ?>
          <div class="btn btn-danger"><i class="icon-notification"></i> <?php echo $this->utilities->getReadableStatus($this->group); ?></div>
        <?php endif; ?>   
        ...
        <?php if ($this->params->get('groupitem_group_members') != '0') :?>  
          ...
          <div class="media-group-members">
            ...
            <?php foreach ($this->members as $id => $item) : ?>   
              <a href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getUserRoute($item->id)); ?>" class="pull-left"><img width="50" height="50" border="0" src="<?php echo JRoute::_($this->utilities->getAvatar($item)); ?>" class="hasTooltip" title="<?php echo JHtml::tooltipText($this->escape($item->name)); ?>" /></a>
            <?php endforeach; ?>              
          </div>
          ...
          <ul class="media-activity-list">
            <?php foreach ($this->activities as $activity) : ?>
              <li class="media-activity-item">
                <a href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getUserRoute($activity->actor)); ?>" class="media-activity-avatar"><img width="50" height="50" border="0" src="<?php echo JRoute::_($this->utilities->getAvatar(JFactory::getUser($activity->actor))); ?>" /></a>
      ...
      <dl class="media-info">
        ...
        <?php if ($this->params->get('list_meta_likes') != '0') :?>
          <dd class="media-info-likes">
            <i class="icon-thumbs-up"></i> <span id="media-likes"><?php echo (int) $this->group->likes; ?></span>
            <i class="icon-thumbs-down"></i> <span id="media-dislikes"><?php echo (int) $this->group->dislikes; ?></span>
          </dd>
        <?php endif; ?>        
        ...
        <?php if ($this->params->get('item_meta_likes') != '0') :?>
          <dd class="media-info-like"><a href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=groups.like&id=' . $this->group->id . '&return=' . $this->return . '&tmpl=component&'.JSession::getFormToken().'=1'); ?>"><?php echo JText::_('COM_HWDMS_LIKE'); ?></a></dd>
          <dd class="media-info-dislike"><a href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=groups.dislike&id=' . $this->group->id . '&return=' . $this->return . '&tmpl=component&'.JSession::getFormToken().'=1'); ?>"><?php echo JText::_('COM_HWDMS_DISLIKE'); ?></a></dd>
        <?php endif; ?>   
        <?php if ($this->params->get('item_meta_report') != '0') :?>
          <dd class="media-info-report"><a title="<?php echo JText::_('COM_HWDMS_REPORT'); ?>" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=groupform.report&id=' . $this->group->id . '&return=' . $this->return . '&tmpl=component'); ?>" class="media-popup-iframe-form"><?php echo JText::_('COM_HWDMS_REPORT'); ?></a></dd>
        <?php endif; ?>          
      </dl>
      <?php endif; ?>  
      <!-- Tags -->
